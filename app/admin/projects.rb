	...
	index do |t|
		...
		t.column "Status" do |project|
			if project.confirmed_at == nil
				link_to("Confirm", admin_notifications_delete_path(project.admin_notification, :path => "/admin/projects", :status => true, :project => project, :type => project.admin_notification.admin_type), :method => :delete) + "  " + link_to("Deny", admin_notifications_delete_path(project.admin_notification, :path => "/admin/projects", :status => false, :project => project, :type => project.admin_notification.admin_type), :method => :delete)
			else
				"CONFIRMED"
			end
		end
